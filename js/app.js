...
function toggleDarkMode() {
  ...
  if (darkMode) {
    ...
    document.body.style.backgroundColor = '#18120D';
...
function renderPeopleGrid(filteredData = null) {
  ...
  data.forEach(person => {
    ...
    card.className = `family-card bg-white border border-[#E5DFD3] rounded-3xl overflow-hidden cursor-pointer flex flex-col dark:bg-[#241B14] dark:border-[#3D2C22]`;
    card.innerHTML = `
      <div class="relative">
      ...
      <div class="p-5 flex-1 flex flex-col">
        <div class="flex-1">
          ...
          <p class="text-[#8B5E3C] text-sm mt-0.5 dark:text-[#D4B78A]">${person.birth} — ${person.death || 'Present'}</p>
          
          ${person.birthPlace ? `<p class="text-xs text-[#6B5B4F] mt-2 line-clamp-1 dark:text-[#C9B8A0]"><i class="fa-solid fa-map-marker-alt mr-1.5"></i>${person.birthPlace}</p>` : ''}
          
          <p class="text-sm text-[#3F2A1E] mt-3 line-clamp-3 dark:text-[#F5EDE0]">${person.bio ? person.bio.substring(0, 140) + (person.bio.length > 140 ? '...' : '') : ''}</p>
        </div>
        
        <div class="mt-4 pt-4 border-t border-[#F5F0E6] flex items-center justify-between text-xs dark:border-[#3D2C22]">
          <div class="flex items-center gap-x-1.5 text-[#8B5E3C] dark:text-[#C9B8A0]">
            <i class="fa-solid fa-users"></i>
            <span>${(person.children || []).length} children</span>
          </div>
          <button class="text-[#4A3728] hover:text-[#8B5E3C] font-medium flex items-center gap-x-1 transition-colors dark:text-[#F5EDE0]">
...
function renderFamilyConnections(person) {
  ...
  if (person.parents && person.parents.length > 0) {
    ...
    parentsContainer.innerHTML = '<p class="text-xs text-[#8B5E3C]/70 italic dark:text-[#C9B8A0]">Unknown / Not documented</p>';
  }
  ...
  if (person.spouses && person.spouses.length > 0) {
    ...
    spousesContainer.innerHTML = '<p class="text-xs text-[#8B5E3C]/70 italic dark:text-[#C9B8A0]">None documented</p>';
  }
  ...
  if (person.children && person.children.length > 0) {
    ...
    childrenContainer.innerHTML = '<p class="text-xs text-[#8B5E3C]/70 italic dark:text-[#C9B8A0]">No children documented</p>';
  }
}

function createRelationshipElement(person, relationType) {
  const div = document.createElement('div');
  div.className = `flex items-center gap-3 p-2.5 bg-[#F5F0E6] hover:bg-[#EDE6D9] rounded-2xl cursor-pointer transition-colors dark:bg-[#32261C] dark:hover:bg-[#3D2C22]`;
  div.innerHTML = `
    <img src="${person.photo}" alt="${person.name}" class="w-9 h-9 rounded-xl object-cover border border-[#D1C7B8]">
    <div class="flex-1 min-w-0">
      <div class="font-medium text-sm truncate dark:text-[#F5EDE0]">${person.name}</div>
      <div class="text-[10px] text-[#6B5B4F] dark:text-[#C9B8A0]">${person.birth} — ${person.death || 'Present'}</div>
    </div>
  `;
...
function createTreeNode(person, isRoot = false) {
  ...
  node.className = `person-node ${isRoot ? 'ring-2 ring-[#8B5E3C] ring-offset-2' : ''} dark:bg-[#241B14] dark:border-[#3D2C22] dark:text-[#F5EDE0]`;
  node.innerHTML = `
    <img src="${person.photo}" alt="${person.name}">
    <div class="node-info flex-1">
      <div class="name dark:text-[#F5EDE0]">${person.name}</div>
      <div class="years dark:text-[#C9B8A0]">${person.birth} — ${person.death || 'Present'}</div>
    </div>
    
    ${(person.children && person.children.length > 0) ? `
      <button class="ml-auto w-6 h-6 flex items-center justify-center text-[#8B5E3C] hover:bg-[#F5F0E6] rounded-full transition-colors toggle-btn dark:text-[#C9B8A0] dark:hover:bg-[#3D2C22]" title="Toggle children">
